...
# Manually set IOC_STARTUP_DIR if zzz.sh will reside somewhere other than ioczzz
#!IOC_STARTUP_DIR=/home/username/epics/ioc/synApps/zzz/iocBoot/ioczzz
...
checkpid() {
    MY_UID=`${ID} -u`
    # The '\$' is needed in the pgrep pattern to select zzz, but not zzz.sh
...
start() {
	...
	screenpid
        ...
        cd ${IOC_STARTUP_DIR}
	# Run zzz inside a screen session
...
run() {
	...
	screenpid
        ...
        ${ECHO} "Starting ${IOC_NAME}"
        cd ${IOC_STARTUP_DIR}
	# Run zzz outside of a screen session, which is helpful for debugging
	${IOC_CMD}
    fi
}
